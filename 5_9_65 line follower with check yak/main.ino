...
void trackline_R(int turn_speed, int line)
{
    ...
    while (count < line)
    {
        trackline_1(fd_speed, turn_speed, turn_ms);
        if (R2_value == 0 && R1_value == 0)
        {
            while (R2_value == 0 && R1_value == 0)
            {
                fd(fd_speed, fd_speed);
                readSensor();
            }
...
void trackline_L(int turn_speed, int line)
{
    ...
    {
        trackline_1(fd_speed, turn_speed, turn_ms);
        if (L2_value == 0 && L1_value == 0)
        {
            while (L2_value == 0 && L1_value == 0)
            {
                fd(fd_speed, fd_speed);
                readSensor();
            }
